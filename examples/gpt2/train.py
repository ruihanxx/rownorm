...
def setup_ddp(args):
    """
    Initialize the distributed environment (when launched with torchrun).
    Returns: device, ddp, rank, local_rank, world_size, master_process, seed_offset.
    """
...
def main():
    ...
    if master_process:
        ...
        # ===== Key change: use "opt_model_name_lr" as the file name and append on each run =====
        lr_str = f"{args.lr:g}"  # e.g. 0.0006 -> '0.0006', 6e-4 -> '0.0006'
        csv_filename = f"tpp20_{args.model_name}_{args.opt}_{lr_str}_p{args.p_exp}_{args.use_fan_scaling}_20.csv"
        csv_path = os.path.join(logs_dir, csv_filename)

        # If the file already exists, append to it; otherwise create it and write the header
